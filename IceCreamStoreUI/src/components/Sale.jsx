...
function Sale() {
  ...
  const items = [
    { id: 1, dateHour: data.toLocaleDateString() + " : " + data.toLocaleTimeString(), table: 1, itens: ["Copinho 1 Bola", "Cascão 2 Bolas"], totalPrice: 17.00 },
    { id: 2, dateHour: data.toLocaleDateString() + " : " + data.toLocaleTimeString(), table: 5, itens: ["Copinho 2 Bolas", "Cascão 1 Bola"], totalPrice: 17.00 },
  ];

  const filteredItems = items.filter((item) => {
    const matchesSearch = appliedFilter === "" ? true : item.table == appliedFilter;
    ...
    <div className="ml-0 md:ml-40 h-[calc(100vh-40px)] bg-purple-backgroundbody flex flex-col">
      ...
      <div className="flex flex-col items-center pt-7 gap-7 relative">
        <div className="flex w-[90%] h-10 justify-between">
          <button onClick={() => setAppliedFilter(filter)} className="bg-purple-items border w-[10%] text-gray-300 rounded-md cursor-pointer">
            Pesquisar
          </button>
          <input
            type="text"
            ...
            <div className="grid grid-rows-auto">
              {filteredItems.map((item) => (
                <div key={item.id} className="grid grid-cols-4 border-b border-purple-border text-white">
                  ...
                  <div className="px-4 py-2">{item.itens}</div>
                  <div className="px-4 py-2 text-right">R$ {item.totalPrice}</div>
                </div>
              ))}
